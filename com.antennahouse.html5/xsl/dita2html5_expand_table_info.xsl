...
<!--
    **************************************************************
    ...
    Module: Build normalized table information
    **************************************************************
    Copyright © 2008-2021 Antenna House, Inc. All rights reserved.
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="gColNamePrefix" as="xs:string" static="yes" select="'col'"/>
    ...
    <!-- 
     ...
     note:      Newly added attributes for entry:
                ...
                @ahf:signiture   :    Express entry hierarchy string 
                @ahf:col-name    :    Express entry column name 
     -->
    ...
    <xsl:template match="*[@class => contains-token('topic/entry')]" mode="MODE_EXPAND_COLUMN_SPAN">
        <xsl:param name="prmColSpec" tunnel="yes" required="yes" as="element()+"/>
        ...
        <xsl:copy>
            ...
            <xsl:attribute name="ahf:colname" select="$gColNamePrefix || string($colnum)"/>
            ...
            <!-- Record entry hierarchy sign -->
            <xsl:attribute name="ahf:signiture" select="ahf:getHistoryStr(.)"/>
        </xsl:copy>
        <xsl:if test="$colSpanCount gt 0">
            <xsl:for-each select="1 to $colSpanCount">
                <entry class=" topic/entry " ahf:colnum="{string($colnum + position())}" ahf:colname="{$gColNamePrefix || string($colnum + position())}" ahf:col-spanned="yes">
                    <xsl:if test="$rowSpanCount gt 0">
                        <xsl:attribute name="ahf:row-span-count" select="string($rowSpanCount)"/>
                    </xsl:if>
                </entry>
    ...
    <xsl:template match="*[@class => contains-token('topic/entry')]" mode="MODE_COPY_ENTRY">
        ...
        <xsl:copy>
            ...
            <xsl:if test="$prmColNum ne $entry/@dita-ot:x => ahf:nz()">
                <xsl:message select="ahf:replace($stMes2011,('%file', '%path', '%dita-ot:x', '%ahf:colnum'),($entry/@xtrf => string(), ahf:getHistoryStr($entry), $entry/@dita-ot:x => string(), $prmColNum => string()))"/>
            </xsl:if>
        </xsl:copy>
    </xsl:template>
    ...
    <xsl:function name="ahf:getColNumFromColName" as="xs:integer">
        ...
        <!-- DEBUG -->
        <xsl:variable name="colnum" as="xs:integer?" select="xs:integer($prmColSpec[string(@colname) eq $prmColName]/@colnum)"/>
        ...
        <xsl:sequence select="xs:integer($prmColSpec[string(@colname) eq $prmColName]/@colnum)"/>
    </xsl:function>
    ...
    <!-- 
     function:  Entry template for complementing column span
     param:		
     return:    entry
