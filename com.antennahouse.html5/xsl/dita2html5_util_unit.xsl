...
<xsl:stylesheet version="3.0" 
    ...
    <xsl:variable name="cPxDpi"   as="xs:integer">
    ...
    <xsl:function name="ahf:toIn" as="xs:double" visibility="public">
        ...
        <xsl:try>
            <xsl:choose>
                <xsl:when test="ends-with($prmUnitValue,$cUnitPc)">
                    <xsl:sequence select="xs:double(xs:double(substring($prmUnitValue, 1, string-length($prmUnitValue) - string-length($cUnitPc))) * $cPtPerPc div $cPtPerIn)"/>
                </xsl:when>
                <xsl:when test="ends-with($prmUnitValue,$cUnitPt)">
                    <xsl:sequence select="xs:double(substring($prmUnitValue, 1, string-length($prmUnitValue) - string-length($cUnitPt))) div $cPtPerIn"/>
                </xsl:when>
                <xsl:when test="ends-with($prmUnitValue,$cUnitPx)">
                    <xsl:sequence select="xs:double(xs:double(substring($prmUnitValue, 1, string-length($prmUnitValue) - string-length($cUnitPx))) div $cPxDpi)"/>
                </xsl:when>
                <xsl:when test="ends-with($prmUnitValue,$cUnitIn)">
                    <xsl:sequence select="xs:double(substring($prmUnitValue, 1, string-length($prmUnitValue) - string-length($cUnitIn)))"/>
                </xsl:when>
                <xsl:when test="ends-with($prmUnitValue,$cUnitCm)">
                    <xsl:sequence select="xs:double(xs:double(substring($prmUnitValue, 1, string-length($prmUnitValue) - string-length($cUnitCm))) div $cCmPerIn)"/>
                </xsl:when>
                <xsl:when test="ends-with($prmUnitValue,$cUnitMm)">
                    <xsl:sequence select="xs:double(xs:double(substring($prmUnitValue, 1, string-length($prmUnitValue) - string-length($cUnitMm))) div $cMmPerCm div $cCmPerIn)"/>
                </xsl:when>
                <xsl:when test="ends-with($prmUnitValue,$cUnitTwip)">
                    <xsl:sequence select="xs:double(xs:double(substring($prmUnitValue, 1, string-length($prmUnitValue) - string-length($cUnitTwip))) div $cTwipPerPt  div $cPtPerIn)"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:call-template name="warningContinue">
                        <xsl:with-param name="prmMes" select="ahf:replace($stMes2000,('%value'),($prmUnitValue))"/>
    ...
    <!--
    function:   Convert to pixel
    param:      prmUnitVal any numeric value with length unit
    return:     the computed pixel value
    note:       
    -->
    <xsl:function name="ahf:toPx" as="xs:double" visibility="public">
        <xsl:param name="prmUnitValue" as="xs:string"/>
        <xsl:param name="prmDpi" as="xs:integer"/>
        <xsl:sequence select="ahf:toIn($prmUnitValue) * $prmDpi"/>
    </xsl:function>
